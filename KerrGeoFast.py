...
from KerrParameters import KerrParameters_1
from KerrParameters import KerrParameters_2

class KerrGeodesic_1:
    def __init__(
        self,
        a: float = 0.,
        r1: float = 50,
        ...
        self.N = N
        para = KerrParameters_1(self.a,self.z1,self.r1,self.r2).parameter()
        self.E = para["E"]
        self.L = para["L"]
        self.C = para["C"]
        self.r1 = para["r1"]
    ...
    def angular_frequencies(self) -> None:
        ...
        self.Tr = 2*np.pi/self.Upsilon_r
        self.Upsilon_z = (np.pi * self.z2) / (2 * self.K_k_z)
        self.Tz = 2*np.pi/self.Upsilon_z
        # print(f"Tr={self.Tr}")
    ...
    def evolution_parameters(self) -> None:
        ...
        self.q_r = self.Upsilon_r * self.lambda_array[:round(self.Tr/self.delta_lambda)]
        self.q_z = self.Upsilon_z * self.lambda_array[:round(self.Tz/self.delta_lambda)]
    ...
    def r_and_z(self) -> None:
        ...
        self.r_lambda = np.resize(numerator_r / denominator_r,self.N)

        # 计算 z(lambda)
        self.z_lambda = np.resize(self.z1 * self.sn_z,self.N)

    def t_and_phi(self) -> None:
        """计算 t(lambda) 和 phi(lambda)。"""
        # 计算 t_r(q_r), t_z(q_z), phi_r(q_r), phi_z(q_z)
        t_r = np.resize(self.t_r(self.q_r),self.N)
        t_z = np.resize(self.t_z(self.q_z),self.N)
        phi_r = np.resize(self.phi_r(self.q_r),self.N)
        phi_z = np.resize(self.phi_z(self.q_z),self.N)
...
class KerrGeodesic_2:
    def __init__(
        ...
        self.delta_lambda = delta_lambda
        self.N = N
        para = KerrParameters_2(self.a,self.E,self.C,self.L).parameter()
        self.E = para["E"]
        self.L = para["L"]
        self.C = para["C"]
        self.r1 = para["r1"]
